    ...
    <div className="relative inline-block" ref={ref}>
      ...
      <div>
        ...
        >
          ...
          {selectedOption}
          <svg
            className={`absolute top-0 right-0 h-5 w-5 mt-10 mr-3 transition-transform duration-300 ${isOpen ? 'transform rotate-180' : ''
              }`}
          ...
          <div className="py-1">
            {options.map((option, index) => (
              <button
                ...
                className={`block w-full h-[50px] text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 text-left ${index === 0 ? 'border-t' : ''} ${index === options.length - 1 ? 'border-b' : ''}`}
              >
                {option}
              </button>
            ))}
